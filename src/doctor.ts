  ...
  if (hasConfig) {
    try {
      ...
      if (!isRecord(product)) {
        findings.push({
          code: "product.preset",
          message: `products.crossPlatform missing (expected preset, default ${DEFAULT_PRESET})`,
          severity: "error",
        });
      } else if (typeof product.preset !== "string" || !product.preset) {
        findings.push({
          code: "product.preset",
          message: `products.crossPlatform.preset missing (expected e.g. ${DEFAULT_PRESET})`,
          severity: "error",
        });
      } else {
        findings.push({
          code: "product.preset",
          message: `products.crossPlatform.preset=${product.preset}`,
          severity: "info",
        });
      }
    } catch (err) {
