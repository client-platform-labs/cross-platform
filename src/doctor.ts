...
export async function runDoctor(cwd: string): Promise<DoctorFinding[]> {
  ...
  findings.push({
    ...
    message: hasConfig
      ...
      : `missing ${CONFIG_FILE_NAME} (run cross-platform init)`,
  ...
  findings.push({
    ...
    message: hasManifest
      ...
      : `missing ${MANIFEST_FILE_NAME} (run cross-platform init)`,
  ...
  if (hasConfig) {
    try {
      ...
      if (!product) {
        findings.push({
          code: "product.crossPlatform",
          message: `products.crossPlatform invalid (expected preset/sharedCore/targets, default ${DEFAULT_PRESET})`,
          severity: "error",
        });
      } else {
        const experimental = product.targets.filter((t) => t.support === "experimental");
        findings.push({
          code: "product.crossPlatform",
          message: `targets=${product.targets.map((t) => t.id).join(",")} experimental=${experimental.length}`,
          severity: experimental.length ? "warn" : "info",
        });
      }
    } catch (err) {
      const message =
